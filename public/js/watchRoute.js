...
function init() {
  ...
  async function authenticator() {
    const route = await fetch("/route/get/route/by/id", {
      ...
      body: JSON.stringify({
        route_id: new URL(window.location.href).searchParams.get("route_id"),
      }),
    });
  ...
  async function loadPage() {
    ...
    document.querySelector("#routename").value =
      edit_route.data.RoutesHistories[0].route_name;
    document.querySelector("#description").value =
      edit_route.data.RoutesHistories[0].route_description;
    document.querySelector(".addresname").value =
      edit_route.data.Points[0].point_data[0].name;
    document.querySelector(".addres").value =
      edit_route.data.Points[0].point_data[0].addres;
    ...
    edit_route.data.RoutesHistories[0].route_images
      .split(",")
      .forEach((photo) => {
  ...
  savebtn.onclick = async () => {
    ...
    for (let i = 0; i < saveNodeList.length; i += 2) {
      ...
      await ymaps
        .geocode(
          saveNodeList[i + 1].value.slice(
            saveNodeList[i + 1].value.indexOf("#") == -1
              ? 0
              : saveNodeList[i + 1].value.indexOf("#") + 1,
            -1
          )
    ...
    formData.append(
      "status",
      document.querySelector("#status").checked ? "public" : "private"
    );
...
async function render_history() {
  ...
  const active_route_history_response = await fetch(
    "/route/get/route/history/by/id",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  active_route_history.RoutesHistories =
    active_route_history.RoutesHistories.filter(
      (elem) => elem.route_status == "old"
    );
  active_route_history.Points = active_route_history.Points.filter(
    (elem) => elem.point_status == "old"
  );
  ...
  active_route_history.RoutesHistories.forEach((history_route, history_id) => {
    ...
    history_version.onclick = () => {
      ...
      document.querySelector("#description").value =
        history_route.route_description;
      document.querySelector(".addresname").value =
        history_point.point_data[0].name;
      document.querySelector(".addres").value =
        history_point.point_data[0].addres;
