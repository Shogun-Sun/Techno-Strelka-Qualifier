  ...
  router.post("/route/upload/new/route/images", uploadRouteImages.array("file"), async (req, res) => {
    ...
    router.get('/route/get/all/public/routes/names', async (req, res) => {
      try{
        ...
        res.status(200).json({message: 'Названия маршрутов успешно получены', data: routes_name});
      }catch(err){
    ...
    router.post('/route/get/route/by/id', async (req, res) => {
      ...
      try{
        ...
        res.status(200).json({message: 'Маршрут успешно получен', data: data_route});
      }catch(err){
      ...
      try{
        const photos_routes = await Routes.findOne({
         ...
         for(let i = 0; i<pr.length; i++){
          routeData.push(`http://localhost:3000/storages/routeImages/${route_id}/${pr[i]}`)
         }
        res.status(200).json({message: 'Фото успешно отправлены', data: routeData});
